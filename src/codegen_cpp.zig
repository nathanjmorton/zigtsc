...
pub const CodeGenCpp = struct {
    ...
    pub fn deinit(self: *CodeGenCpp) void { self.out.deinit(self.allocator); }
    ...
    fn emitClassImpl(self: *CodeGenCpp, node: Node, class_names: []const []const u8) !void {
        ...
        // Include other class headers used in method bodies
        for (class_names) |cn| {
            if (!std.mem.eql(u8, cn, name)) {
    ...
    fn emitVarDecl(self: *CodeGenCpp, node: Node) !void {
        ...
        try self.writeIndent();
        const type_s = if (type_node != null_node) self.typeStr(type_node) else "auto";
